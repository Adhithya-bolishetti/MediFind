...
import com.medifind.user.controller.UserController;
import com.medifind.notification.dto.UserResponse;
...
public class AdminNotificationController {
    ...
    public ResponseEntity<List<NotificationResponse>> broadcast(@RequestBody SendRequest request) {
        ...
        if ("PATIENT".equals(recipient)) {
            targets = objectMapper.convertValue(userController.getUsersByRole("PATIENT").getBody(), new com.fasterxml.jackson.core.type.TypeReference<List<UserResponse>>() {});
        } else if ("DOCTOR".equals(recipient)) {
            targets = objectMapper.convertValue(userController.getUsersByRole("DOCTOR").getBody(), new com.fasterxml.jackson.core.type.TypeReference<List<UserResponse>>() {});
        } else {
            targets = objectMapper.convertValue(userController.getUsersByRole("ALL").getBody(), new com.fasterxml.jackson.core.type.TypeReference<List<UserResponse>>() {});
        }
