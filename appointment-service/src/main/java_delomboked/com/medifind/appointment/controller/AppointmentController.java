...
public class AppointmentController {
    ...
    @PostMapping
    public ResponseEntity<AppointmentResponse> createAppointment(@Valid @RequestBody AppointmentRequest request, @RequestAttribute(value = "X-User-Id", required = false) Long userId) {
    ...
    public ResponseEntity<AppointmentResponse> cancelAppointment(@PathVariable Long id, @RequestAttribute(value = "X-User-Id", required = false) Long userId) {
        if (userId == null) userId = 1L;
        return ResponseEntity.ok(appointmentService.cancelAppointment(id, userId));
    }

    @PutMapping("/{id}/confirm")
    public ResponseEntity<AppointmentResponse> confirmAppointment(@PathVariable Long id, @RequestAttribute(value = "X-User-Id", required = false) Long doctorId) {
        // simplified: assuming user ID is doctor ID
        if (doctorId == null) doctorId = 1L;
        return ResponseEntity.ok(appointmentService.confirmAppointment(id, doctorId));
    }

    @PutMapping("/{id}/complete")
    public ResponseEntity<AppointmentResponse> completeAppointment(@PathVariable Long id, @RequestAttribute(value = "X-User-Id", required = false) Long doctorId) {
        if (doctorId == null) doctorId = 1L;
    ...
    public ResponseEntity<Boolean> hasCompletedAppointment(@PathVariable Long doctorId, @RequestParam Long userId) {
    ...
    public ResponseEntity<List<String>> getBookedSlots(@PathVariable Long doctorId, @RequestParam String date) {
